CREATE OR REPLACE PROCEDURE load_project_locations_2023
IS
    /**********************************************************************
    ...
    03-22-2024 SH Revise to keep history by year, add snapshot_year column
               SH Procedure to add 2023 project locations (run once)
    06-21-2024 SH Add check to join from projex to ensure asset types match to handle the case where the asset_id for different asset types is the same (ie bridge and node have the same number)
    **********************************************************************/
    ...
    g_jobname      VARCHAR2 (30) := 'LOAD_PROJECT_LOCATIONS_2023';
    ...
    IS
        SELECT psn,                   -- Road, Rail, Trail
          ...
          FROM capital_projects_linear@gis c, complete_transportation_network ctn
         WHERE     c.rtcode = ctn.route_number
               AND ctn.begin_section_mp >= c.beg_rlm_nbr
               AND ctn.end_section_mp <= c.end_rlm_nbr
               AND ctn.row_type = 'Element'
                     and ctn.snapshot_year = 2023
 
        UNION 
          SELECT psn,                           -- In service bridges
               i.section_id,
               i.element_id_on_structure as element_id,
               i.offset       AS begin_section_offset,
               i.offset       AS end_section_offset,
               ...
               c.beg_rlm_nbr,
               c.end_rlm_nbr
          FROM capital_projects_point@gis c join ibridges_history i on c.segment_id = i.bridge_number 
     WHERE c.seg_type_cde = 'BRIDGE' AND i.state = 'PAST' AND  i.asset_status = 1 and i.snapshot_year = 2023 ; 
        ...
        SELECT c.proj_seq,      -- Rail Crossings, Traffic Signals, Nodes    
               ...
               p.work_status
          from v_gis_project_locations@projprd c, GIS_ASSET_INFO@projprd a, wh_projex.db_model_pm_project p 
         where a.asset_type IN ('RRX', 'TRAFFIC_SIGNAL','NODE')and a.end_dt IS NULL  and  a.asset_id = c.segment_id and a.asset_type = c.asset_type and c.proj_seq = p.proj_seq;
          
         cursor large_culvert_projects is
...
BEGIN
    ...
    LOOP
        INSERT INTO PROJECT_LOCATIONS (asset_number,
             ...
             VALUES (ps.asset_number,
                     ps.asset_type,
                     ps.begin_section_offset,
                     ps.begin_section_mp,
                     ps.element_id,
                     ps.end_section_offset,
                     ps.end_section_mp,
                     ps.est_cost,
                     ps.section_id,
                     ps.beg_rlm_nbr,
 ...
 FOR x IN xx_sections
    LOOP
       P_GET_ELEMENT_SECTION_2023(x.route_number, x.milepoint, telement_id, toffset, tsection_id,  tbegin_section_mp,
    tend_section_mp );
    /*
...
FOR lc IN large_culvert_projects
    LOOP
       P_GET_ELEMENT_SECTION_2023(lc.route_number, lc.beg_rlm_nbr, telement_id, toffset, tsection_id,  tbegin_section_mp,
    tend_section_mp ); 
             INSERT INTO PROJECT_LOCATIONS (asset_number,
                                               asset_type,
                                               begin_offset,
