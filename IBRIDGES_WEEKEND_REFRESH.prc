CREATE OR REPLACE PROCEDURE ibridges_weekend_refresh (
...
IS
    ...
    for the weekly refresh.  New bridges are added.  Old bridges are 'RETIRED'.
   ...
   9-7-16 SH - Add columns from special bridge_inspections   FC_INSPECTION_REQUIRED, FC_INSPECTION_FREQUENCY, FC_LAST_INSPECTION
    ...
    12-5-2016 SH - split refresh into ibridges_daily_refresh and ibridges_weekend_refresh to fix problem where new bridge could only be added on the weekend
                   Remove comparison of columns from metrans which are only updated on the weekend from daily refresh:
    ...
    12-9-16 SH - add posting and ratings columns that can be updated via a from independent of an inspection
     ...
     1-4-17   SH -  Add execute load_bridge_program_history , check error log
      ...
      02-17-17 SH - Add the metrans offset back in.  Needed to calculate the location of the bridge on primary/alternate routes - Ed Beckwith request
                  ...
                  Add milepoint in this procedure.  Offset gets refreshed on the weekend.  Milepoint gets refreshed daily.
    ...
    05-15-17  SH  Add the most recent inspection of anytype, column: last_inspection_anytype
    ...
    05-19-17 SH  Invoke the procedure,  monitor_routine_inspections to write inconsistencies with routine inspection dates to the warehouse alert log
    06-06-17 SH  Rearrange order procedures invoked to fix timing problem
                 do not compare columns that get updated after the refresh:  inspection_type, inspection_type_descr, last_routine_inspecton_date, last_element_inspection,
   ...
   05-24-2018 SH Invoke  monitor_bridge_changes Send an alert report when the following changes are made in InspectTech.
    ...
    11-13-19 SH  Add proposed_bridges and archived_bridges load/update procedures: update_archived_bridges; load_proposed_bridges;
    ...
    12-13-19 SH  Add column, MTRNS_ASSETNO to support bridge pointers for bridge_underclearance portals
    12-22-19 SH Stop running load_proposed_bridges.  It is now a view
    ...
    **********************************************************************/
    ...
    g_jobname                VARCHAR2 (30) := 'IBRIDGES_WEEKEND_REFRESH';
...
BEGIN
    ...
    THEN
        ...
        -- weekends, refresh bridge information from InspectTech and location data from METRANS
        load_ibridge_staging;
        load_ibridges_staging_location;
        load_linear_bridge_locations;
        load_ibridge_staging_scores;
        Order_ibridge_staging_towns;
        ibridge_roads_weekly_refresh;
        Load_ibridge_staging_priority;
    END IF;
                ...
                BEGIN
                    UPDATE ibridges_history
                       SET date_modified = start_time,
                           modified_by = 'IBRIDGES_WEEKEND_REFRESH',
                           ...
                           placecode_descr = rec.placecode_descr,
                           posted_bridge_indicator =
                           ...
                           towncode = rec.towncode,
                           towncode2 = rec.towncode2,
                     ...
                     WHERE     bridge_number = rec.bridge_number
                       ...
                       LOG ERRORS INTO ibridges_history_error_log
                               (   'IBridges weekend Refresh - Update bridge: '
                                || SYSDATE)
                               REJECT LIMIT 100;
                ...
                BEGIN
                    ...
                    INSERT INTO ibridges_history (
                             ...
                             VALUES (surrogate_key,
                                     ...
                                     NULL                    /*date_modified*/
                                         ,
                                     'IBRIDGES_WEEKEND_REFRESH' /*created_by*/
                                                               ,
                                     NULL                       /* end_date */
                                         ,
                                     NULL                    /* modified_by */
                            ...
                            LOG ERRORS INTO IBRIDGES_HISTORY_ERROR_LOG
                                    (   'INSERT IBRIDGES WEEKEND REFRESH '
                                     || SYSDATE)
                                    REJECT LIMIT 100;
