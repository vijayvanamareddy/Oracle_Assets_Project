CREATE OR REPLACE PROCEDURE load_project_locations
IS
    /**********************************************************************
    ...
    03-27-2024 SH Revised to keep history by year, add snapshot_year column
                  Fix begin_section_mp and end_section_mp for Large_culverts - getting it from complete transportation network instead of using milepoint of culvert
    ...
    IS
        ...
        SELECT psn,                          -- Archived Bridges
         ...
         SELECT psn,                  -- LARGE CULVERTS                  
               ...
               ctn.begin_section_mp    AS begin_section_mp,
               ctn.end_section_mp    AS end_section_mp,
               est_cost,
               rtcode         AS route_number,
               c.seg_type_cde AS asset_type,
               c.segment_id   AS asset_number,
               ...
               c.beg_rlm_nbr,
               c.end_rlm_nbr
          FROM capital_projects_point@gis c, LARGE_CULVERTS l, complete_transportation_network ctn
         WHERE seg_type_cde = 'LARGE_CULVERT' AND c.segment_id = l.ASSET_SYS_ID and l.section_id = ctn.section_id and l.route_number = ctn.route_number;
/*  
       SELECT psn,                  -- LARGE CULVERTS     - 3/28/24 change             
               l.section_id,
               l.element_id,
               l.offset       AS begin_offset,
               l.offset       AS end_offset,
...
BEGIN
    ...
    snapshot_yr := F_Get_snapshot_year;
    delete from project_locations where snapshot_year = snapshot_yr;
    commit;
