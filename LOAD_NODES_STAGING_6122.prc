CREATE OR REPLACE PROCEDURE load_nodes_staging_6122
IS
    /***************************************************************************************
    ...
    ****************************************************************************************/
    ...
    CURSOR crsh
    IS
        SELECT node AS NODE_ID, mev, node_type FROM nodes@crash;
    ...
    CURSOR aadt
    IS
          SELECT node_id, SUM (factored_aadt) AS faadt
            FROM (SELECT DISTINCT n.node_id,
                                  n.element_id,
                                  n.section_id,
                                  h.one_way,
                                  h.one_way_descr,
                                  (s.factored_aadt / 2) factored_aadt
                    FROM nodes_on_route n
                         JOIN elements h ON n.element_id = h.element_id
                         JOIN sections s ON n.section_id = s.section_id
                   WHERE h.one_way IN (1, 2) AND n.route_type <> 'F' -- When a section of road is one way, divide the faadt by 2
                  UNION
                  (SELECT DISTINCT n.node_id,
                     ...
                     FROM nodes_on_route n
                          JOIN elements h ON n.element_id = h.element_id
                          JOIN sections s ON n.section_id = s.section_id
                    WHERE h.one_way = 0 AND n.route_type <> 'F')) -- When a section of road is not one way, use the entire faadt
        GROUP BY node_id;
...
BEGIN
    ...
    LOOP
        UPDATE nodes_staging n
           SET n.mev = c.mev, n.node_type = c.node_type
    ...
    LOOP
        UPDATE nodes_staging n
           SET n.factored_aadt = a.faadt
         WHERE n.node_id = a.node_id
           LOG ERRORS INTO nodes_error_log
                   ('Update NODE FAADT ' || SYSDATE)
                   REJECT LIMIT 100;
